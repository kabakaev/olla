...
cleanup() {
    ...
    pkill -f "otel_http_prefix_proxy.py" || true
    # Force kill if needed
    lsof -ti:$OLLA_PORT | xargs kill -9 2>/dev/null || true
    lsof -ti:$BACKEND_PORT | xargs kill -9 2>/dev/null || true
    lsof -ti:$HTTP_PROXY_PORT | xargs kill -9 2>/dev/null || true
}
trap cleanup EXIT
...
ensure_mock_backend() {
    ...
    echo -e "${YELLOW}Mock backend not running, restarting...${RESET}"
...
for transport in "${TRANSPORTS[@]}"; do
    ...
    case "$transport" in
        grpc)
            ...
            ;;
        http)
            otlp_endpoint="127.0.0.1:$HTTP_PROXY_PORT"
            otlp_path="/api/default/otel"
            ;;
        *)
    ...
    if [[ "$transport" == "http" ]]; then
        python3 "$SCRIPT_DIR/otel_http_prefix_proxy.py" "$HTTP_PROXY_PORT" &
        wait_for_port $HTTP_PROXY_PORT "OTLP HTTP Prefix Proxy"
    fi
    ...
    OLLA_TELEMETRY_OTLP_PATH="$otlp_path" \
