...
func applyEnvOverrides(config *Config) {
	...
	if val := os.Getenv("OLLA_TELEMETRY_OTLP_PATH"); val != "" {
		config.Telemetry.OTLP.Path = val
	}
	...
	if val := os.Getenv("OLLA_TELEMETRY_OTLP_SKIP_HEALTH_TRACES"); val != "" {
		if enabled, err := strconv.ParseBool(val); err == nil {
			config.Telemetry.OTLP.SkipHealthTraces = enabled
		}
	}
